...
function jstackwithcpu(){
    ...
    stacklog=$(jstack $pid 2>&1 | awk -v RS= -v ORS='\n\n' '!/Full thread dump|Threads class SMR|JNI global refs/')
...
cat $jstacklog|sed -E -e 's/0x[0-9a-z]+/0x00/g' -e '/^[0-9.]+ ::: "/ s/[0-9]+/n/3g' \
    |awk -v RS="" -v FS=":::" '{s[$2]["num"]++;s[$2]["cpu"]+=$1} END{for(k in s){cpu=s[k]["cpu"];num=s[k]["num"];printf "%-3s ::: %-3s ::: %3s\n\n",cpu,num,k}}' \
    |awk -v RS="" -v FS=":::" 'gsub(/\n/,"\\n",$0)||1' \
    |sort -nrk1 \
    |sed 's/$/\n/;s/\\n/\n/g' \
    |less -S
